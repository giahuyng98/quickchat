...
private:
    ...
    msg::User createUserImpl(const srv::CreateUserRequest *req) {
        ...
        model::User user(id, req->username(), req->email(), req->fullname(), password);
        query.insert(user);
        query.execute();
    ...
    msg::User loginByUsername(const std::string &username, const std::string &password) {
    ...
    msg::User loginByEmail(const std::string &email, const std::string &password) {
        constexpr auto QUERY = "SELECT users.* FROM users WHERE username = %0q";
        mysqlpp::Query query = conn.query(QUERY);
        query.parse();
